...
sudo apt-get install -y curl build-essential autoconf libncurses5-dev openssl libssl-dev libpam0g-dev
...
if [ -d /opt/erlang/${ERL_VERSION} ]; then
    ...
    if [ -s $ERL_TARBALL_BASENAME ]; then
        ...
        sudo kerl install ${ERL_VERSION} /opt/erlang/${ERL_VERSION}
